...
if __name__=='__main__':
    ...
    #encoder
    x=Conv2D(6,(5,5),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.01),name='convA_5x5')(input_img)
    x=LeakyReLU(alpha=0.1)(x)
    x=MaxPooling2D((5,5),padding='same',data_format='channels_first',name='pool5x5')(x)
    logging.debug(str(x.shape))


    x=Conv2D(4,(4,4),strides=(2, 2), padding='valid',data_format='channels_first',name='convB_4x4_stride2',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))

    x=Conv2D(4,(4,4),strides=(2, 2), padding='valid',data_format='channels_first',name='convC_4x4_stride2',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    x=Dropout(0.25)(x)
    x=Flatten()(x)
