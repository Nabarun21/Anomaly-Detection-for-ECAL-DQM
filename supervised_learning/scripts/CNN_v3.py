...
if __name__=='__main__':
    ...
    logging.debug("Current training file list is "+str(train_data_list)+" and has "+str(len(train_data_list))+" files")
    logging.debug("Current test file list is "+str(test_data_list)+" and has "+str(len(test_data_list))+" files")
    ...
    (_,image_height,image_width)=get_data(file_list[0],image_type)[0].shape
    ...
    #encoder
    x=Conv2D(12,(5,5),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.05),name='convA_5x5')(input_img)
    x=LeakyReLU(alpha=0.1)(x)
    x=MaxPooling2D((5,5),padding='same',data_format='channels_first',name='pool5x5')(x)
    logging.debug(str(x.shape))


    x=Conv2D(8,(4,4),strides=(2, 2), padding='valid',data_format='channels_first',name='convB_4x4_stride2',kernel_regularizer=regularizers.l2(0.02))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))

    x=Conv2D(6,(4,4),strides=(2, 2), padding='valid',data_format='channels_first',name='convC_4x4_stride2',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    x=Conv2D(6,(4,4), padding='same',data_format='channels_first',name='convD_4x4',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    ...
    print(x)
    logging.debug(str(x.shape))
    x=Dense(100,name='denseA')(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.5)(x)
    logging.debug(str(x.shape))
    output=Dense(2,name='denseB',activation='softmax')(x)
    logging.debug(str(output.shape))


    CNN=Model(input_img, output)

    CNN.compile(optimizer=args.opt_name, loss=args.loss_name,metrics=['accuracy'])
...
#        np.random.shuffle(train_data_list)
        ...
        for batch in my_generator:
            ...
            CNN.fit(train_batch,test_batch,epochs=1,batch_size=40,shuffle=True,validation_split=0.25,callbacks=[training_history,early_stopping,checkpoint])
        ...
        if 'EB' not in args.img_type.upper():
            ...
            for batch_eem in my_generator_eem:
                CNN.fit(batch_eem,batch_eem,epochs=1,batch_size=40,shuffle=True,validation_split=0.25,callbacks=[training_history,early_stopping])
                batchwise_loss_history.extend(training_history.batchwise_losses)
                gen_batch_loss_history.extend(training_history.epochwise_losses)
                gen_batch_val_loss_history.extend(training_history.epochwise_val_losses)
    ...
    CNN.save(os.environ['BASEDIR']+"/models/CNN_"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+'.h5')
