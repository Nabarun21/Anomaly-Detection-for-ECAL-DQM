...
import logging
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D,LeakyReLU,Flatten,Dropout,BatchNormalization
...
if __name__=='__main__':
    ...
    x=Conv2D(4,(4,4),strides=(2, 2), padding='valid',data_format='channels_first',name='convB_4x4_stride2')(x)
    x=BatchNormalization(axis=1)(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))

    x=Conv2D(4,(4,4),strides=(2, 2), padding='valid',data_format='channels_first',name='convC_4x4_stride2')(x)
    x=BatchNormalization(axis=1)(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    x=Dropout(0.25)(x)
