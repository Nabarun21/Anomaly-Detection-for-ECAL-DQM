...
def get_roc_curve(models, working_point=None):
    """Generates ROC Curves for a given array"""
    fig, ax = plt.subplots()

    for legend_label, model_scores in models:
...
if __name__=="__main__":
    ...
    for  batch in my_training_data_generator:
        ...
        predicted=trained_model.predict(train_x)
        y_true_train=train_y[:,0]
        y_predicted_train=predicted[:,0]
    ...
    with open(os.environ['BASEDIR']+"/models/"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+"_for_metrics_training.txt", "wb") as fp:   #Pickling
