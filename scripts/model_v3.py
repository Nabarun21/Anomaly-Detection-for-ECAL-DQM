...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D,LeakyReLU,Flatten,Dropout,Reshape
...
parser.add_argument(
      ...
      dest="loss_name",
      default="MSE",
      help="name of the loss fucntion to be used")
parser.add_argument(
      "--log_level",
      action="store",
      dest="log_level",
...
if args.int_log:
    ...
    logging.basicConfig(level=logging_numeric_level, format='%(asctime)s %(levelname)s: %(message)s',filename=os.environ["BASEDIR"]+'/logs/'+args.model_name+'_'+args.loss_name+'_'+args.opt_name+'.log')
#logging.basicConfig(level=logging_numeric_level, format='%(asctime)s %(levelname)s: %(message)s')
...
if __name__=='__main__':
    ...
    train_data_list=file_list[0:47] #choosing 63 here keeps ~80% of data for testing, rest for training and val, need to automatize this
    val_data_list=file_list[47:63] #choosing 63 here keeps ~80% of data for testing, rest for training and val, need to automatize this
    ...
    #encoder
    x=Conv2D(32,(5,5),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.02))(input_img)
    x=LeakyReLU(alpha=0.1)(x)
    x=MaxPooling2D((5,5),padding='same',data_format='channels_first')(x)
    logging.debug(str(x.shape))


    x=Conv2D(16,(4,4),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.02))(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=MaxPooling2D((2,2),padding='same',data_format='channels_first')(x)
    logging.debug(str(x.shape))

    x=Conv2D(12,(2,2),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=MaxPooling2D((2,3),padding='same',data_format='channels_first')(x)
    logging.debug(str(x.shape))

    x=Conv2D(4,(2,3),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    ...
    x=Flatten()(x)
    logging.debug(str(x.shape))
    x=Dense(50,name='denseA')(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.3)(x)
    logging.debug(str(x.shape))
    x=Dense(432,name='denseB')(x)
    x=Dropout(0.3)(x)
    ...
    x=UpSampling2D((2,3),data_format='channels_first')(x)
    x=Conv2D(8,(2,2),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    x=Conv2D(12,(2,1),padding='valid',data_format='channels_first',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    ...
    x=UpSampling2D((5,5),data_format='channels_first')(x)
    x=Conv2D(32,(5,5),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.02))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))

    decoded=Conv2D(1,(5,5),activation='sigmoid',padding='same',data_format='channels_first')(x)
    logging.debug(str(x.shape))
    ...
    logging.info("Current validation set is made from "+str(len(val_data_list))+" files and has "+str(get_num_samples(val_data_list,image_type))+" examples")
    ...
    checkpoint=callbacks.ModelCheckpoint(os.environ['BASEDIR']+"/models/best_model_"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+'.h5',save_best_only=True)

    for epoch in range(num_epochs):
        np.random.shuffle(train_data_list)
        np.random.shuffle(val_data_list)
        logging.info("Epoch no %s",epoch+1)
        my_generator=batch_generator(1000,train_data_list,image_type)
        my_val_generator=batch_generator(250,val_data_list,image_type)
        ...
        for batch in my_generator:
            try:
                ...
                my_val_generator=batch_generator(250,val_data_list,image_type)
                val_batch=my_val_generator.next()
            autoencoder.fit(batch,batch,epochs=1,batch_size=50,shuffle=True,validation_data=(val_batch, val_batch),callbacks=[training_history,early_stopping,checkpoint])
        ...
        if 'EB' not in args.img_type.upper():
            ...
            my_generator_eem=batch_generator(600,train_data_list,image_type_eem)
            for batch_eem in my_generator_eem:
                autoencoder.fit(batch_eem,batch_eem,epochs=1,batch_size=250,shuffle=True,validation_split=0.25,callbacks=[training_history,early_stopping])
                batchwise_loss_history.extend(training_history.batchwise_losses)
                gen_batch_loss_history.extend(training_history.epochwise_losses)
                gen_batch_val_loss_history.extend(training_history.epochwise_val_losses)
    ...
    with open(os.environ['BASEDIR']+"/models/"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+"_batchwise_loss.txt", "wb") as fp:   #Pickling
        pickle.dump(batchwise_loss_history, fp)
    with open(os.environ['BASEDIR']+"/models/"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+"_epochwise_val_loss.txt", "wb") as fp:   #Pickling
