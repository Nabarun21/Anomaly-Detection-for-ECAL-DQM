...
if __name__=='__main__':
    ...
    #encoder
    x=Conv2D(4,(20,20),padding='same',data_format='channels_first')(input_img)
    ...
    x=Conv2D(4,(2,2),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=MaxPooling2D((2,3),padding='same',data_format='channels_first')(x)
    logging.debug(str(x.shape))

    x=Conv2D(4,(2,3),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    ...
    x=Conv2D(4,(2,2),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Conv2D(4,(2,1),padding='valid',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    ...
    x=Conv2D(4,(20,20),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))

    decoded=Conv2D(1,(20,20),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
...
#        np.random.shuffle(train_data_list)
        ...
        my_generator=batch_generator(600,train_data_list,image_type)
        gen_batch_loss_history=[]
        gen_batch_val_loss_history=[]

        logging.info("Training on EB or EE+ images")
        for batch in my_generator:
            autoencoder.fit(batch,batch,epochs=1,batch_size=40,shuffle=True,validation_split=0.25,callbacks=[training_history,early_stopping,checkpoint])
        ...
        if 'EB' not in args.img_type.upper():
            ...
            for batch_eem in my_generator_eem:
                autoencoder.fit(batch_eem,batch_eem,epochs=1,batch_size=40,shuffle=True,validation_split=0.25,callbacks=[training_history,early_stopping])
                batchwise_loss_history.extend(training_history.batchwise_losses)
                gen_batch_loss_history.extend(training_history.epochwise_losses)
                gen_batch_val_loss_history.extend(training_history.epochwise_val_losses)
    ...
    autoencoder.save(os.environ['BASEDIR']+"/models/autoencoder_"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+'.h5')
