...
if __name__=='__main__':
    ...
    train_data_list=file_list[0:54] #choosing 63 here keeps ~80% of data for testing, rest for training and val, need to automatize this
    val_data_list=file_list[54:63] #choosing 63 here keeps ~80% of data for testing, rest for training and val, need to automatize this
    ...
    #encoder
    x=Conv2D(4,(5,5),padding='valid',strides=(5,5),data_format='channels_first',use_bias=False)(input_img)
    ...
    x=Conv2D(6,(4,4),padding='same',strides=(2,2),data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.3)(x)
    x=Dropout(0.1)(x)
    logging.debug(str(x.shape))
    ...
    x=Conv2D(6,(2,4),padding='valid',strides=(2,3),data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.3)(x)
    x=Dropout(0.1)(x)
    ...
    x=Conv2D(4,(17,18),padding='same',data_format='channels_first',use_bias=False)(x)                
    x=Dropout(0.1)(x)
    x=BatchNormalization()(x)                                                                        
    logging.debug(str(x.shape))
    
    x=UpSampling2D((2,4),data_format='channels_first')(x)
    x=Conv2D(2,(4,4),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=Dropout(0.1)(x)
    x=LeakyReLU(alpha=0.3)(x)
    logging.debug(str(x.shape))
