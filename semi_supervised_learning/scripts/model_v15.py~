...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D,LeakyReLU,Dropout
...
if __name__=='__main__':
    ...
    my_opt=optimizers.SGD(lr=0.015,decay=2e-7)
    #get the data
    try:
        data_folder=os.environ["DATA"]
    except KeyError:
    ...
    #encoder
    x=Conv2D(12,(5,5),padding='same',data_format='channels_first')(input_img)
    ...
    x=Conv2D(8,(2,2),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.25)(x)
    ...
    x=Conv2D(6,(2,3),padding='same',data_format='channels_first')(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.1)(x)
