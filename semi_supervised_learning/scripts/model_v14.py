...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D,LeakyReLU,Dropout,Flatten,Reshape,BatchNormalization
...
if __name__=='__main__':
    ...
    my_opt=optimizers.SGD(lr=0.03,decay=4e-7)
    #get the data
    try:
        data_folder=os.environ["DATA"]
    except KeyError:
    ...
    #encoder
    x=Conv2D(8,(5,5),padding='same',data_format='channels_first')(input_img)
    ...
    x=Conv2D(6,(4,4),padding='same',data_format='channels_first')(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.25)(x)
    ...
    x=Conv2D(6,(2,2),padding='same',data_format='channels_first')(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.25)(x)
...
#    encoded=MaxPooling2D((1,1),padding='same',data_format='channels_first')(x)
    ...
    x=Conv2D(8,(2,1),padding='valid',data_format='channels_first')(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.25)(x)
    ...
    x=Conv2D(8,(4,4),padding='same',data_format='channels_first')(x)
    x=BatchNormalization()(x)
    ...
    x=Conv2D(6,(5,5),padding='same',data_format='channels_first')(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.3)(x)
    logging.debug(str(x.shape))
...
#    encoder=Model(input_img,encoded)
    ...
    for epoch in range(num_epochs):
        ...
        my_generator=batch_generator(2000,train_data_list,image_type)
        my_val_generator=batch_generator(500,val_data_list,image_type)
