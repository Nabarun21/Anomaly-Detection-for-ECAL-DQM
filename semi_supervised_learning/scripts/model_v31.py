...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D,LeakyReLU,Dropout,BatchNormalization
...
def get_data(file_name,group='EBOccupancyTask_EBOT_rec_hit_occupancy',data_type='good'):
   ...
   ret_array_norm=max_scale(normalize(ret_array))
...
if __name__=='__main__':
    ...
    x=MaxPooling2D((5,5),padding='same',data_format='channels_first')(input_img)
    logging.debug(str(x.shape))


    x=Conv2D(4,(4,4),padding='same',data_format='channels_first',use_bias=False)(x)
    ...
    x=Conv2D(4,(2,2),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.3)(x)
    x=Dropout(0.1)(x)
    ...
    x=Conv2D(4,(2,1),padding='valid',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.3)(x)
    x=Dropout(0.1)(x)
    ...
    x=Conv2D(3,(4,4),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=Dropout(0.15)(x)
    x=LeakyReLU(alpha=0.3)(x)
    logging.debug(str(x.shape))
    ...
    decoded=Conv2D(1,(5,5),activation='tanh',padding='same',data_format='channels_first',use_bias=False)(x)
    logging.debug(str(x.shape))
