...
if __name__=='__main__':
    ...
    #encoder
    encoder_layer_1=Conv2D(4,(3,3),activation='relu',padding='same',data_format='channels_first')(input_img)
    logging.debug(str(encoder_layer_1.shape))


    decoder_layer_1=Conv2D(4, (3, 3), activation='relu', padding='same',data_format='channels_first')(encoder_layer_1)
    logging.debug(str(decoder_layer_1.shape))

    if 'EB' in args.img_type.upper(): 
        decoder_final=Conv2D(1,(3,3),activation='sigmoid',data_format='channels_first',padding='same')(decoder_layer_1)
...
#        np.random.shuffle(train_data_list)
        ...
        for batch in my_generator:
            autoencoder.fit(batch,batch,epochs=1,batch_size=40,shuffle=True,validation_split=0.25,callbacks=[training_history,early_stopping])
    ...
    autoencoder.save(os.environ['BASEDIR']+"/models/"+args.model_name+'_'+args.loss_name+'_'+args.opt_name+'.h5')
