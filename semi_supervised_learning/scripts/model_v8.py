...
if __name__=='__main__':
    ...
    my_opt=optimizers.SGD(lr=0.5,decay=1.25e-5)
    #get the data
    try:
        data_folder=os.environ["DATA"]
    except KeyError:
    ...
    x=Conv2D(8,(2,3),padding='same',data_format='channels_first',kernel_regularizer=regularizers.l2(0.01))(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
    ...
    autoencoder.compile(optimizer=my_opt, loss=args.loss_name)
    ...
    for epoch in range(num_epochs):
        ...
        my_generator=batch_generator(1050,train_data_list,image_type)
