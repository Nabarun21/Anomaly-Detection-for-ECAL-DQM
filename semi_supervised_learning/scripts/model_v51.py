...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D,LeakyReLU,Dropout,BatchNormalization,Flatten,Reshape
...
def get_data(file_name,group='EBOccupancyTask_EBOT_rec_hit_occupancy',data_type='good'):
   ...
   ret_array_norm=normalize(ret_array)
   logging.debug("Supplying "+str(ret_array_norm.shape[0])+" samples")
   return ret_array_norm
...
if __name__=='__main__':
    ...
    mask_busy_gpus(leave_unmasked=1)
    ...
    #encoder
    x=Conv2D(8,(5,5),padding='same',data_format='channels_first',use_bias=False)(input_img)
    ...
    x=Conv2D(8,(4,4),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.2)(x)
    ...
    x=Conv2D(8,(2,2),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.15)(x)
    ...
    x=Flatten()(x)
    logging.debug(str(x.shape))
    x=Dense(108,name='denseA')(x)
    x=LeakyReLU(alpha=0.2)(x)
    x=Dropout(0.2)(x)
    logging.debug(str(x.shape))


    x=Dense(50,name='denseB')(x)
    x=LeakyReLU(alpha=0.2)(x)
    logging.debug(str(x.shape))
    x=Dense(108,name='denseC')(x)
    x=LeakyReLU(alpha=0.2)(x)
    logging.debug(str(x.shape))
    x=Reshape((1,9,12))(x)
    

    x=Conv2D(6,(2,3),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.1)(x)
    ...
    x=Conv2D(6,(2,2),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.1)(x)

    x=Conv2D(8,(2,1),padding='valid',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=LeakyReLU(alpha=0.1)(x)
    x=Dropout(0.15)(x)
    ...
    x=Conv2D(6,(5,5),padding='same',data_format='channels_first',use_bias=False)(x)
    x=BatchNormalization()(x)
    x=Dropout(0.2)(x)
    x=LeakyReLU(alpha=0.1)(x)
    logging.debug(str(x.shape))
